...
export const AppHeader = () => {
  ...
  const activeContentType =
    location.pathname === '/' ? (searchParams.get('type') as ContentType | null) : null
  ...
  const filterItemClassName = (active: boolean) =>
    cn(
  ...
  return (
    <>
      <header className="sticky top-0 z-30 border-b border-border/70 bg-card/86 backdrop-blur supports-backdrop-filter:bg-card/78">
        <div className="mx-auto flex w-full max-w-420 items-center justify-between px-4 py-2 sm:px-6 lg:px-8">
          ...
          <div ref={searchAreaRef} className="flex flex-1 h-4 items-center justify-end gap-2">
            ...
            {session?.user ? (
              <>
                <Link to="/favorites" className={navItemClassName(favoritesActive)}>
                  <Star className="size-4" />
                  收藏
                </Link>
                <Separator orientation="vertical" className="opacity-50" />
                <Link to="/subscriptions" className={navItemClassName(subscriptionsActive)}>
